...
const Divider = forwardRef<HTMLDivElement, DividerProps>(
  ({ children, className, gradian, ...rest }: DividerProps, ref) => {
    ...
    if (children) {
      return (
        <div ref={ref} className='flex items-center'>
          <div
            className={twMerge(
              'flex-grow my-2 border-t border-t-divider dark:border-t-gray-600',
              gradian && gradianthemeLeft,
              className
            )}
            {...rest}
          />
    ...
    return (
      <div
        className={twMerge('my-2 border-t border-t-divider dark:border-t-gray-600', gradian && gradiantheme, className)}
        {...rest}
      />
    )
  }
