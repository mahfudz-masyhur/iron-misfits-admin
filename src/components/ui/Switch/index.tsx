...
const Switch = forwardRef<HTMLInputElement, SwitchProps>((props, ref) => {
  const { label, sizes, color, className, ...rest } = props
  const disabled = rest.disabled

  return (
    <label className='relative inline-flex items-center cursor-pointer'>
      <input ref={ref} type='checkbox' className='sr-only peer' {...rest} />
      <div className={twMerge(variants({ color, sizes, className }), disabled && 'peer-checked:bg-bg-gray-300')} />
      {label && (
        <span className={twMerge('ml-3 text-sm font-medium', disabled && 'text-gray-300 dark:text-gray-500')}>
          {label}
        </span>
      )}
    </label>
